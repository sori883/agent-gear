    ...
    if (Array.isArray(patterns)) {
      ...
      for (let candidate = resolve(directory); candidate !== parent; candidate = dirname(candidate)) {
        ...
        if (includes.some(pattern => new Bun.Glob(pattern.replace(/\/$/, "")).match(path)) && !excludes.some(pattern => new Bun.Glob(pattern.slice(1).replace(/\/$/, "")).match(path))) {
          throw new Error(`natural-japanese scripts belongs to an ancestor workspace (${join(parent, "package.json")}); place this skill outside its workspace patterns before installing dependencies.`);
        }
      }
    }
...
export async function bootstrap(directory = import.meta.dir): Promise<boolean> {
  ...
  if ((version[0] ?? 0) < 1 || version[0] === 1 && ((version[1] ?? 0) < 4 || version[1] === 4 && (version[2] ?? 0) < 2)) throw new Error("natural-japanese requires Bun 1.4.2 or later");
  const directoryModules = join(directory, "node_modules");
  const marker = join(directoryModules, ".natural-japanese-bootstrap.json");
  const lock = join(directoryModules, ".natural-japanese-bootstrap.lock");
  ...
  if (!handle) throw new Error("natural-japanese dependency bootstrap is locked; inspect scripts/node_modules/.natural-japanese-bootstrap.lock");
  ...
  try {
    ...
    if (code !== 0) throw new Error(`natural-japanese dependency installation failed (${code}): ${stderr || stdout}`);
    await Promise.all(Object.keys(manifest.dependencies ?? {}).map(name => access(join(directoryModules, name, "package.json"))));
