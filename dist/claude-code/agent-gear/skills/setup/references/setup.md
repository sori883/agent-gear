...
| 内容 | 配置先 |
| --- | --- |
| 共通知識・ルール | `.space/babel/vendor/<plugin>/` |
...
指示テンプレートの`{{SKILL_ROOT}}`は配布ルートのskills、`{{VENDOR_BUNDLE}}`は利用先のvendor bundleの絶対パスに置き換わる。プラグインの配置場所が変わった場合もplanで差を確認し、applyで参照を更新する。
