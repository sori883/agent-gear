...
const boundedLimit = (limit: number) => limit <= 0 ? 10 : Math.min(limit, 100);
...
export function searchForPath(b: Bundle, target: string, query = "", limit = 10): SearchResult[] {
  ...
  return results.sort((a, b) => rank(b.governance) - rank(a.governance) || b.score - a.score || compare(a.concept_id, b.concept_id)).slice(0, boundedLimit(limit));
}
